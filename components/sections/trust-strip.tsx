  ...
  return (
    <section aria-label="At a glance" className="border-y border-border bg-surface/40">
      <div className="container-page">
        <Reveal>
          <ul className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl bg-border sm:grid-cols-4">
            {trustItems.map((item) => {
              const Icon = icons[item.icon];
              return (
                <li
                  key={item.label}
                  className="flex items-center justify-center gap-2.5 bg-bg px-4 py-5 text-center"
                >
